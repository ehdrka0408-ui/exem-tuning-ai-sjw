...
def _capture_after(inst: dict, sql: str, schema: Optional[str], binds: list[BindVar], index_ddls: Optional[list[str]] = None) -> AfterCapture:
    ...
    try:
        ...
        logger.warning(f"[marker-debug] marker={marker} tagged_sql_head={tagged_sql[:200]!r}")
        wall_t0 = time.time()
        try:
        ...
        try:
            cur.execute(
                ...
                "WHERE sql_text LIKE :m AND ROWNUM = 1",
                {"m": f"%{marker}%"},
            )
            row = cur.fetchone()
            if row:
            ...
            logger.warning(f"after v$sql marker lookup failed: {e}")
        if captured_sql_id is None:
            try:
                ...
                if row:
                    captured_sql_id = row[0]
                    captured_child = row[1]
                    logger.info("after sql_id: marker miss, fallback to prev_sql_id")
            except Exception as e:
                logger.warning(f"after v$session fallback lookup failed: {e}")
        ...
        gv_sql_hit = False
        stats_sql_id = captured_sql_id
        if stats_sql_id is None:
            try:
                cur.execute(
                    "SELECT prev_sql_id FROM v$session WHERE sid = SYS_CONTEXT('USERENV','SID')")
                row = cur.fetchone()
                stats_sql_id = row[0] if row else None
            except Exception as e:
                logger.warning(f"after stats prev_sql_id fallback failed: {e}")
        try:
            if stats_sql_id:
                cur.execute(
                ...
                if r and r[0] is not None:
                    ...
                    gv_sql_hit = True
                    logger.warning(f"after gv$sql stats: sql_id={stats_sql_id} executions={executions}")
        except Exception as e:
            logger.warning(f"after gv$sql stats lookup failed: {e}")
        ...
        if xplan["elapsed_sec"] is not None:
            ...
            logger.warning(f"after xplan id=0: elapsed={xplan['elapsed_sec']} buffers={xplan['buffers']}")
...
def _improvement(before: BeforePerformance, after: BeforePerformance,
                 before_plan_hash: Optional[str] = None,
                 after_plan_hash: Optional[str] = None,
                 ) -> tuple[Optional[float], Optional[float]]:
    # plan_hash 동일 또는 executions 비율 5배 초과 → totals 직접 비교 (per-exec 오염 방지)
    same_plan = (before_plan_hash and after_plan_hash
                 and before_plan_hash == after_plan_hash)
    bex = before.executions_count or 0
    aex = after.executions_count or 0
    exec_skewed = bex > 0 and aex > 0 and aex / bex > 5
    use_totals = same_plan or exec_skewed
    if use_totals:
        b_el = before.elapsed_time_sec
        a_el = after.elapsed_time_sec
        b_buf = before.buffer_gets_count
        a_buf = after.buffer_gets_count
    else:
        b_el = (before.elapsed_time_sec / bex) if bex > 0 and before.elapsed_time_sec is not None else before.elapsed_time_sec
        a_el = (after.elapsed_time_sec / aex) if aex > 0 and after.elapsed_time_sec is not None else after.elapsed_time_sec
        b_buf = (before.buffer_gets_count / bex) if bex > 0 and before.buffer_gets_count is not None else before.buffer_gets_count
        a_buf = (after.buffer_gets_count / aex) if aex > 0 and after.buffer_gets_count is not None else after.buffer_gets_count
    e_pct = None
    if b_el and b_el > 0 and a_el is not None:
        e_pct = (b_el - a_el) / b_el
    b_pct = None
    if b_buf and b_buf > 0 and a_buf is not None:
        b_pct = (b_buf - a_buf) / b_buf
    if use_totals:
        logger.warning(f"_improvement: use_totals=True same_plan={same_plan} exec_skewed={exec_skewed} "
                       f"before_ex={bex} after_ex={aex} e_pct={e_pct}")
    return e_pct, b_pct


def _same_plan_hash(before_hash: Optional[str], after_hash: Optional[str]) -> bool:
...
def _upsert_to_be_sql(db: Session, *, sql_text_str: str, schema_name: Optional[str],
    ...
    db.execute(
        sql_text(
            ...
            "VALUES (:sid, :iid, :txt, 'to_be') RETURNING sql_id"
        ),
...
def _normalize_sql(sql: str) -> str:
    """공백 정규화 후 소문자 — 동등성 비교용."""
    return re.sub(r'\s+', ' ', sql.strip().lower()) if sql else ''
    ...
    *,
    ...
    for attempt in range(1, MAX_ATTEMPTS + 1):
        if deadline is not None and time.time() > deadline:
            logger.warning(f"[timeout] pipeline deadline exceeded at attempt={attempt}, request_id={request_id}")
            rat = f"[timeout:{int(deadline - time.time() + (deadline - time.time() if deadline else 0))}s 초과] 파이프라인 타임아웃"
            return _finish_failed(db, request_id, attempt - 1,
                                  f"[timeout] 파이프라인 실행 제한 시간 초과 (attempt={attempt})")
        ctx = TuningContext(
            sql_text=as_is_sql,
        ...
        # 이전 시도 SQL 과 공백 정규화 후 동일하면 즉시 재시도
        _prev_sqls = [p.get("tuned_sql") for p in (previous_attempts or []) if p.get("tuned_sql")]
        if any(_normalize_sql(last_tuned) == _normalize_sql(ps) for ps in _prev_sqls):
            logger.warning(f"[retune] attempt={attempt} TO-BE identical to previous attempt SQL — forcing retry")
            last_failure_note = (
                f"\n\n[재시도 {attempt}] 직전 응답이 이전 튜닝 시도와 동일한 SQL 입니다. "
                "힌트(INDEX/NO_INDEX/PARALLEL/USE_HASH), CTE, 서브쿼리 구조 등 완전히 다른 접근으로 재작성하세요."
            )
            continue
        ...
        _before_phash = _extract_plan_hash(before_plan) if before_plan else None
        _after_phash = _extract_plan_hash(after.plan_text) if after.plan_text else None
        e_pct, b_pct = (_improvement(before_perf, after.perf,
                                     before_plan_hash=_before_phash,
                                     after_plan_hash=_after_phash)
                        if after.perf else (None, None))
        ...
        # plan_hash 동일이면 물리적 경로 변화 없음 → 개선없음으로 처리
        if _same_plan_hash(_before_phash, _after_phash):
            logger.warning(
                f"[no_improve] attempt={attempt} same_plan_hash={_before_phash} — physical path unchanged"
            )
            last_failure_note = (
                f"\n\n[재시도 {attempt}] 실행계획(plan_hash={_before_phash})이 변경되지 않았습니다. "
                "버퍼 캐시 warmup 효과로 elapsed 가 줄어 보일 수 있으나 실제 개선 아님. "
                "반드시 다른 액세스 경로(다른 인덱스/조인 방법/힌트)를 사용하는 SQL 을 생성하세요."
            )
            continue
...
def _persist_after_snapshot(db: Session, *, request_id: int, instance_id: int,
                            sql_id: str,
    ...
    if plan:
        db.execute(
            sql_text(
                ...
                "VALUES (:iid, :sid, 'after', :pt, :phash)"
                " ON CONFLICT (sql_id, instance_id, phase, plan_hash) DO NOTHING"
            ),
            {"iid": instance_id, "sid": sql_id, "pt": plan, "phash": _extract_plan_hash(plan)},
        )
    if perf:
        ...
        db.execute(
            sql_text(
                ...
                "VALUES (:rid, :iid, :sid, 'after', :el, :cp, :bg, :dr, :ex, :rp, :ie, :ph)"
            ),
            {"rid": request_id, "iid": instance_id, "sid": sql_id,
             ...
             "ie": is_estimated, "ph": _extract_plan_hash(plan)},
        )
    db.commit()


def _finish_completed(db: Session, *, request_id: int, tobe_sql_id: str,
    ...
    db.execute(
        sql_text(
            "UPDATE tuning_request_group SET "
            "  llm_id=(SELECT llm_id FROM llm_models WHERE provider=:prov AND model_name=:mdl LIMIT 1) "
            "WHERE group_id=(SELECT group_id FROM tuning_requests WHERE request_id=:rid LIMIT 1)"
        ),
        {"prov": llm.provider, "mdl": llm.model, "rid": request_id},
    )
    db.execute(
        sql_text(
...
def _finish_failed(db: Session, request_id: int, attempts: int, rationale: str) -> JobOutcome:
    db.execute(
        sql_text(
...
def _finish_failed_with_after(db: Session, *, request_id: int, attempts: int, rationale: str,
    ...
    if last_llm is not None:
        db.execute(
            ...
            {"prov": last_llm.provider, "mdl": last_llm.model, "rid": request_id},
        )
        db.execute(
            sql_text(
