...
def create_tuning_request(body: TuningRequestBody, db: Session = Depends(get_db)) -> TuningRequestResponse:
    ...
    # 2) tuning_requests INSERT — request_id 는 시퀀스 기반 생성
    request_id = _generate_request_id(db)
    db.execute(
        sql_text(
            "INSERT INTO tuning_requests "
            "(request_id, instance_id, asis_sql_id, source, status, user_instruction, requested_by, parent_request_id) "
            "VALUES (:rid, :iid, :sid, :src, 'requested', :ui, :rb, :pid)"
        ),
    ...
    for bv in body.binds:
        db.execute(
            sql_text(
                "INSERT INTO sql_bind_variables (sql_id, instance_id, name, data_type, value, position) "
                "VALUES (:sid, :iid, :n, :dt, :v, :p)"
            ),
            {"sid": asis_sql_id, "iid": instance_id, "n": bv.name, "dt": bv.data_type,
    ...
    if perf:
        db.execute(
            sql_text(
                ...
                "VALUES (:rid, :iid, :sid, 'before', :el, :cp, :bg, :dr, :ex, :rp)"
            ),
...
def list_tuning_requests(
    ...
    q = f"""
        ...
        LEFT JOIN db_instances di ON di.id = r.instance_id
        {where_clause}
...
def get_tuning_request(request_id: str, db: Session = Depends(get_db)) -> dict:
    ...
    row = db.execute(
        sql_text(
            ...
            "       requested_at, completed_at, instance_id "
    ...
    bind_rows = db.execute(
        sql_text("SELECT name, data_type, value, position FROM sql_bind_variables "
                 "WHERE sql_id = :sid AND instance_id = :iid ORDER BY position, captured_at"),
        {"sid": base["asis_sql_id"], "iid": instance_id},
...
def apply_tuning_request(request_id: str, body: ApplyBody = ApplyBody(),
    ...
    bind_rows = db.execute(
        ...
        {"sid": rec["asis_sql_id"], "iid": instance_id},
    ).fetchall()
    ...
    if cap.perf:
        db.execute(
            sql_text(
                ...
                "(request_id, instance_id, sql_id, phase, elapsed_time_sec, cpu_time_sec, "
                " buffer_gets_count, disk_reads_count, executions_count, rows_processed_count) "
                "VALUES (:rid, :iid, :sid, 'applied', :el, :cp, :bg, :dr, :ex, :rp)"
            ),
