...
def create_tuning_request(body: TuningRequestBody, db: Session = Depends(get_db)) -> TuningRequestResponse:
    ...
    db.execute(
        sql_text(
            ...
            "(request_id, instance_id, asis_sql_id, source, status, user_instruction, requested_by, parent_request_id, user_id) "
            "VALUES (:rid, :iid, :sid, :src, 'requested', :ui, :rb, :pid, :uid)"
        ),
        {"rid": request_id, "iid": instance_id, "sid": asis_sql_id, "src": body.source,
         "ui": body.user_instruction, "rb": body.requested_by,
         "pid": body.parent_request_id, "uid": body.user_id},
    )
